...
st.set_page_config(
    page_title="Bundesliga Analyzer",
    page_icon="⚽",
    layout="wide",
...
df_database = pd.read_csv("./data/data_buli_1320_cleaned.csv")
types = ["Mean", "Absolute", "Median", "Maximum", "Minimum"]
label_attr_dict = {"Goals": "goals", "Halftime Goals": "ht_goals", "Shots on Goal": "shots_on_goal", "Distance Covered (in km)": "distance", "Passes": "total_passes", "Successful Passes": "success_passes", "Failed Passes": "failed_passes",
                   "Pass Success Ratio": "pass_ratio", "Ball Possession": "possession", "Tackle Success Ratio": "tackle_ratio", "Fouls Committed": "fouls", "Fouls Received": "got_fouled", "Offsides": "offside", "Corners": "corners"}
label_attr_dict_teams = {"Goals Scored": "goals", "Goals Received": "goals_received", "Halftime Goals Scored": "ht_goals", "Halftime Goals Received": "halftime_goals_received", "Shots on opposing Goal": "shots_on_goal", "Shots on own Goal": "shots_on_goal_received",
                         "Distance Covered (in km)": "distance", "Passes": "total_passes", "Successful Passes": "success_passes", "Failed Passes": "failed_passes", "Pass Success Ratio": "pass_ratio", "Ball Possession": "possession", "Tackle Success Ratio": "tackle_ratio", "Fouls Committed": "fouls", "Fouls Received": "got_fouled", "Offsides": "offside", "Corners": "corners"}
color_dict = {'1. FC Köln': '#fc4744', '1. FC Nürnberg': '#8c0303', '1. FC Union Berlin': '#edd134', '1. FSV Mainz 05': '#fa2323', 'Bayer 04 Leverkusen': '#cf0c0c', 'Bayern München': '#e62222', 'Bor. Mönchengladbach': '#1f9900', 'Borussia Dortmund': '#fff830', 'Eintracht Braunschweig': '#dbca12', 'Eintracht Frankfurt': '#d10606', 'FC Augsburg': '#007512', 'FC Ingolstadt 04': '#b50300',
...
label_attr_dict_correlation = {"Goals": "delta_goals", "Halftime Goals": "delta_ht_goals", "Shots on Goal": "delta_shots_on_goal",
                               "Distance Covered (in km)": "delta_distance", "Passes": "delta_total_passes", "Pass Sucess Ratio": "delta_pass_ratio", "Possession": "delta_possession", "Tackle Success Ratio": "delta_tackle_ratio", "Fouls": "delta_fouls", "Offside": "delta_offside", "Corners": "delta_corners"}
label_fact_dict = {"goals scored": 'goals', "halftime goals scored": 'ht_goals', "shots on the goal": 'shots_on_goal',
                   "distance covered (in km)": 'distance', "total passes": 'total_passes', "pass ratio": 'pass_ratio', "possession ratio": 'possession', "successful tackle ratio": 'tackle_ratio', "fouls": 'fouls', "offsides": 'offside', "corners": 'corners'}
...
def filter_season(df_data):
    ...
    # get raw start season "13-14"
    start_raw = start_season.replace("/", "-").replace("‏‏‎ ‎‏‏‎ ‎", "")
    # get raw end season "19-20"
...
def filter_matchday(df_data):
    ...
    matchdays_list = list(
        range(selected_matchdays[0], selected_matchdays[1]+1))
...
def stack_home_away_dataframe(df_data):
    ...
    delta_names = ['goals', 'ht_goals', 'shots_on_goal', 'distance', 'total_passes',
                   'pass_ratio', 'possession', 'tackle_ratio', 'fouls', 'offside', 'corners']
    ...
    column_names = ['distance', 'total_passes', 'success_passes', 'failed_passes', 'pass_ratio', 'possession', 'tackle_ratio', 'offside', 'corners', 'delta_goals', 'delta_ht_goals',
                    'delta_shots_on_goal', 'delta_distance', 'delta_total_passes', 'delta_pass_ratio', 'delta_possession', 'delta_tackle_ratio', 'delta_fouls', 'delta_offside', 'delta_corners']
    h_column_names = ['game_id', 'season', 'matchday', 'h_team', 'h_goals', 'a_goals',
                      'h_ht_goals', 'a_ht_goals', 'h_shots_on_goal', 'a_shots_on_goal', 'h_fouls', 'a_fouls']
    a_column_names = ['game_id', 'season', 'matchday', 'a_team', 'a_goals', 'h_goals',
                      'a_ht_goals', 'h_ht_goals', 'a_shots_on_goal', 'h_shots_on_goal', 'a_fouls', 'h_fouls']
    column_names_new = ['game_id', 'season', 'matchday', 'location', 'team', 'goals', 'goals_received', 'ht_goals', 'ht_goals_received', 'shots_on_goal', 'shots_on_goal_received', 'fouls', 'got_fouled', 'distance', 'total_passes', 'success_passes', 'failed_passes',
                        'pass_ratio', 'possession', 'tackle_ratio', 'offside', 'corners', 'delta_goals', 'delta_ht_goals', 'delta_shots_on_goal', 'delta_distance', 'delta_total_passes', 'delta_pass_ratio', 'delta_possession', 'delta_tackle_ratio', 'delta_fouls', 'delta_offside', 'delta_corners']
    ...
    df_total = df_home.append(df_away, ignore_index=True).sort_values(
        ['game_id', 'season', 'matchday'], ascending=[True, True, True])
    df_total_sorted = df_total[['game_id', 'season', 'matchday', 'location', 'team', 'goals', 'goals_received', 'delta_goals', 'ht_goals', 'ht_goals_received', 'delta_ht_goals', 'shots_on_goal', 'shots_on_goal_received', 'delta_shots_on_goal', 'distance', 'delta_distance',
                                'total_passes', 'delta_total_passes', 'success_passes', 'failed_passes', 'pass_ratio', 'delta_pass_ratio', 'possession', 'delta_possession', 'tackle_ratio', 'delta_tackle_ratio', 'fouls', 'got_fouled', 'delta_fouls', 'offside', 'delta_offside', 'corners', 'delta_corners']]
...
def plot_x_per_matchday(attr, measure):
    ...
    ax = sns.barplot(x="aspect", y=attribute,
                     data=df_plot.reset_index(), color="#b80606")
...
def plot_x_per_team(attr, measure):  # total #against, #conceived
    ...
    if specific_team_colors:
        ax = sns.barplot(x="aspect", y=attribute,
                         data=df_plot.reset_index(), palette=color_dict)
    else:
        ax = sns.barplot(x="aspect", y=attribute,
                         data=df_plot.reset_index(), color="#b80606")
...
def plt_attribute_correlation(aspect1, aspect2):
    ...
    if (corr_type == "Regression Plot (Recommended)"):
        ax = sns.regplot(x=asp1, y=asp2, x_jitter=.1, data=df_plot, color='#f21111', scatter_kws={
...
def build_matchfacts_return_string(return_game_id_value_team, min_max, attribute, what):
    ...
    goals_home = str(df_match_result.iloc[0]['goals'])
    string1 = "On matchday " + matchday + " of season " + season + \
        " " + home_team + " played against " + away_team + ". "
    string2 = ""
    if (goals_home > goals_away):
        string2 = "The match resulted in a " + goals_home + ":" + goals_away + \
            " (" + str(df_match_result.iloc[0]['ht_goals']) + ":" + str(
                df_match_result.iloc[1]['ht_goals']) + ") win for " + home_team + "."
    if (goals_home < goals_away):
        string2 = "The match resulted in a " + goals_home + ":" + goals_away + \
            " (" + str(df_match_result.iloc[0]['ht_goals']) + ":" + str(
                df_match_result.iloc[1]['ht_goals']) + ") loss for " + home_team + "."
    if (goals_home == goals_away):
        string2 = "The match resulted in a " + goals_home + ":" + goals_away + \
            " (" + str(df_match_result.iloc[0]['ht_goals']) + ":" + \
            str(df_match_result.iloc[1]['ht_goals']) + ") draw. "
    string3 = ""
    string4 = ""
    value = str(abs(round(return_game_id_value_team[1], 2)))
    team = str(return_game_id_value_team[2])
    if (what == "difference between teams"):
        string3 = " Over the course of the match, a difference of " + \
            value + " " + attribute + " was recorded between the teams."
        string4 = " This is the " + \
            min_max.lower() + " difference for two teams in the currently selected data."
    if (what == "by both teams"):
        string3 = " Over the course of the match, both teams recorded " + \
            value + " " + attribute + " together."
        string4 = " This is the " + \
            min_max.lower() + " value for two teams in the currently selected data."
    if (what == "by a team"):
        string3 = " Over the course of the match, " + team + \
            " recorded " + value + " " + attribute + "."
        string4 = " This is the " + \
            min_max.lower() + " value for a team in the currently selected data."
...
start_season, end_season = st.sidebar.select_slider(
    'Select the season range you want to include', unique_seasons, value=["‏‏‎ ‎‏‏‎ ‎13/14", "19/20‏‏‎ ‎‏‏‎ ‎"])
df_data_filtered_season = filter_season(df_stacked)

### MATCHDAY RANGE ###
unique_matchdays = get_unique_matchdays(
    df_data_filtered_season)  # min and max matchday
selected_matchdays = st.sidebar.select_slider('Select the matchday range you want to include', unique_matchdays, value=[
                                              min(unique_matchdays), max(unique_matchdays)])
df_data_filtered_matchday = filter_matchday(df_data_filtered_season)

### TEAM SELECTION ###
unique_teams = get_unique_teams(df_data_filtered_matchday)
all_teams_selected = st.sidebar.selectbox('Do you want to only include specific teams? If the answer is yes, please check the box below and then select the team(s) in the new field.', [
                                          'Include all available teams', 'Select teams manually (choose below)'])
if all_teams_selected == 'Select teams manually (choose below)':
    selected_teams = st.sidebar.multiselect(
        "Select and deselect the teams you would like to include in the analysis. You can clear the current selection by clicking the corresponding x-button on the right", unique_teams, default=unique_teams)
...
if all_teams_selected == 'Include all available teams':
    row13_spacer1, row13_1, row13_spacer2, row13_2, row13_spacer3, row13_3, row13_spacer4 = st.columns(
        (.2, 2.3, .2, 2.3, .2, 2.3, .2))
    with row13_1:
        show_me_hi_lo = st.selectbox("", ["Maximum", "Minimum"], key='hi_lo')
    with row13_2:
        show_me_aspect = st.selectbox(
            "", list(label_fact_dict.keys()), key='what')
    with row13_3:
        show_me_what = st.selectbox(
            "", ["by a team", "by both teams", "difference between teams"], key='one_both_diff')
    row14_spacer1, row14_1, row14_spacer2 = st.columns((.2, 7.1, .2))
    with row14_1:
        return_game_id_value_team = find_match_game_id(
            show_me_hi_lo, show_me_aspect, show_me_what)
        df_match_result = build_matchfacts_return_string(
            return_game_id_value_team, show_me_hi_lo, show_me_aspect, show_me_what)
    ...
    with row15_3:
        end_result = str(
            df_match_result.iloc[0]['goals']) + " : " + str(df_match_result.iloc[1]['goals'])
        ht_result = " ‎ ‎( " + str(df_match_result.iloc[0]['ht_goals']) + " : " + str(
            df_match_result.iloc[1]['ht_goals']) + " )"
    ...
    with row17_1:
        st.warning(
            'Unfortunately this analysis is only available if all teams are included')

if all_teams_selected == 'Include all available teams':
    row16_spacer1, row16_1, row16_2, row16_3, row16_4, row16_spacer2 = st.columns(
        (0.5, 1.5, 1.5, 1, 2, 0.5))
    with row16_1:
    ...
    with row16_2:
        st.markdown(" ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎" +
                    str(df_match_result.iloc[0]['shots_on_goal']))
        st.markdown(" ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎" +
                    str(df_match_result.iloc[0]['distance']))
        st.markdown(" ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎‎" +
                    str(df_match_result.iloc[0]['total_passes']))
        st.markdown(" ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎‎ ‎‎" +
                    str(df_match_result.iloc[0]['possession']))
        st.markdown(" ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎" +
                    str(df_match_result.iloc[0]['fouls']))
        st.markdown(" ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎" +
                    str(df_match_result.iloc[0]['offside']))
        st.markdown(" ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎" +
                    str(df_match_result.iloc[0]['corners']))
    with row16_4:
        st.markdown(" ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎" +
                    str(df_match_result.iloc[1]['shots_on_goal']))
        st.markdown(" ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎" +
                    str(df_match_result.iloc[1]['distance']))
        st.markdown(" ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎‎" +
                    str(df_match_result.iloc[1]['total_passes']))
        st.markdown(" ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎‎" +
                    str(df_match_result.iloc[1]['possession']))
        st.markdown(" ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎" +
                    str(df_match_result.iloc[1]['fouls']))
        st.markdown(" ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎" +
                    str(df_match_result.iloc[1]['offside']))
        st.markdown(" ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎ ‎" +
                    str(df_match_result.iloc[1]['corners']))
...
with row5_1:
    ...
    plot_x_per_team_selected = st.selectbox("Which attribute do you want to analyze?", list(
        label_attr_dict_teams.keys()), key='attribute_team')
    plot_x_per_team_type = st.selectbox(
        "Which measure do you want to analyze?", types, key='measure_team')
...
with row7_1:
    ...
    plot_x_per_season_selected = st.selectbox("Which attribute do you want to analyze?", list(
        label_attr_dict.keys()), key='attribute_season')
    plot_x_per_season_type = st.selectbox(
        "Which measure do you want to analyze?", types, key='measure_season')
with row7_2:
    if all_teams_selected != 'Select teams manually (choose below)' or selected_teams:
...
### CORRELATION ###
# removed "Violin Plot (High Computation)"
...
with row11_1:
    ...
    corr_type = st.selectbox(
        "What type of correlation plot do you want to see?", corr_plot_types)
