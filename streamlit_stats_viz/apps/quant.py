...
def app():
    ...
    chart_choice = st.sidebar.radio(
        "", ["Histogram", "Boxplot", "Dotplot", "QQplot", "Scatterplot"])
    ...
    if chart_choice == "Histogram":
        ...
        if cv != None:
            p = p + geom_histogram(aes(x=x, fill=cv, color=cv),
                                   position="identity", alpha=.4, bins=bins)
        else:
            p = p + geom_histogram(aes(x=x), color="darkblue",
    ...
    if chart_choice == "Boxplot":
        ...
        if cv != None:
            ...
            p = p + geom_boxplot(aes(x=1, y=x, width=.1),
                                 color="darkblue", fill="lightblue") + coord_flip()
    ...
    if chart_choice == "Dotplot":
        with top[1]:
            ...
            cv = st.selectbox("Color", options=non_numeric_columns)
        if cv != None:
            p = p + geom_jitter(aes(x=cv, y=x, fill=cv, color=cv),
                                size=2, height=0, width=.1) + coord_flip()
        else:
            p = p + geom_jitter(aes(x=1, y=x), size=2,
                                height=0, width=.1) + coord_flip()
    ...
    if chart_choice == "QQplot":
        with top[1]:
            ...
            cv = st.selectbox("Color", options=non_numeric_columns)
        if cv != None:
            p = p + stat_qq(aes(sample=x, color=cv)) + stat_qq_line(aes(sample=x,
                                                                        color=cv)) + labs(x="Theoretical Quantiles", y="Sample Quantiles")
        else:
            p = p + stat_qq(aes(sample=x)) + stat_qq_line(aes(sample=x)) + \
