...
def app():
    ...
    with c2:
        ...
        if xvar != yvar:
            ...
            slope, intercept, r, p, ses = sp.stats.linregress(
                fsdf[xvar], fsdf[yvar])
            ...
            data = pd.DataFrame({"r": r, 'R-Squared': r**2, 'se (resid)': sr, 'Intercept': intercept,
                                'Slope': slope, 'se (slope)': ses, 't (slope)': slope/ses, 'p (slope)': p}, index=[0]).T
            st.write(data)
            lrplot = ggplot(
                aes(x=fsdf[xvar], y=fsdf[yvar])) + coord_fixed(ratio=.1)
            lrplot = lrplot + \
                geom_point(color='steelblue') + stat_smooth(method='lm')
    ...
    with d2:
        ...
        if xvar != yvar:
            ...
            st.markdown(str(
                100*cl)+"'%' confidnce interval for slope: ("+str(slower)+","+str(shigher)+")")
            seyhat = sr*np.sqrt(1+1/n+(gx-meanx)**2/((n-1)*sdx**2))
            pime = ctv*seyhat
            plower = py + pime
            phigher = py - pime
            st.markdown(str(100*cl)+"'%' prediction interval for y when x = " +
                        str(gx)+": ("+str(plower)+","+str(phigher)+")")
            semuhat = sr*np.sqrt(1/n+(gx-meanx)**2/((n-1)*sdx**2))
            cimy = ctv*semuhat
            clower = py + cimy
            chigher = py - cimy
            st.markdown(str(100*cl)+"'%' confidence interval for mean y when x = " +
                        str(gx)+": ("+str(clower)+","+str(chigher)+")")
